...
version: 1.0.0
license: MIT
description: A pipe that calls litellm. Needed to set the custom_metadata
"""
    ...
    async def pipe(
        ...
        **kwargs,
        ...
        def pprint(message: str) -> str:
            print(f"LitellmCaller of '{__user__['name']}': " + str(message))
            return message
        ...
        pprint(f"LitellmCaller User: {__user__}")
        pprint(f"LitellmCaller Body: {body}")
        ...
        try:
            ...
            if "custom_metadata" in payload:
                payload["metadata"] = payload["custom_metadata"]
                del payload["custom_metadata"]
